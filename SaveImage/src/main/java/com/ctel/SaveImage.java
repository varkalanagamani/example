...
import static MvCameraControlWrapper.MvCameraControlDefines.MV_ACCESS_Exclusive;
...
public class SaveImage {
	...
	private static void printDeviceInfo(MV_CC_DEVICE_INFO stDeviceInfo) {
		...
		System.out.println(
				"\tAccessible:      " + MvCameraControl.MV_CC_IsDeviceAccessible(stDeviceInfo, MV_ACCESS_Exclusive));
		System.out.println("");
	}
	...
	public static void saveDataToFile(byte[] dataToSave, int dataSize, String fileName) {
		...
		try {
			// Create directory
			File tempFile = new File("C:\\Users\\ctel\\Documents\\capture");
			if (!tempFile.exists()) {
				tempFile.mkdirs();
			}
				...
				File f = new File(tempFile.getPath() + File.separator + fileName);
				if (f.exists()) {
					System.out.println("\nfile found\n");
				}else {
	...
	public static int chooseCamera(ArrayList<MV_CC_DEVICE_INFO> stDeviceList) {
		...
		if (0 <= camIndex && stDeviceList.size() > camIndex) {
			if (MV_GIGE_DEVICE == stDeviceList.get(camIndex).transportLayerType) {
				System.out.println(
						"Connect to camera[" + camIndex + "]: " + stDeviceList.get(camIndex).gigEInfo.userDefinedName);
			} else if (MV_USB_DEVICE == stDeviceList.get(camIndex).transportLayerType) {
				System.out.println(
						"Connect to camera[" + camIndex + "]: " + stDeviceList.get(camIndex).usb3VInfo.userDefinedName);
			} else {
	...
	public static void main(String[] args) {
		...
		do {
			...
			while (true) {
				...
				stSaveParam.methodValue = 0; // Interpolation method that converts Bayer format to RGB24. 0-Neareast
												// 1-double linear 2-Hamilton
				...
				// String fileNameWithOutExt = FilenameUtils.removeExtension(qrImgName);
				String outputFilename = df.format(new Date());
				// Save buffer content to file
				saveDataToFile(imageBuffer, stSaveParam.imageLen, outputFilename + ".tif");
				count++;
				long end1 = System.currentTimeMillis();
				System.out.println("Elapsed Time in milli seconds: " + (end1 - start1) + "\nElapsed Time in seconds: "
						+ (end1 - start1) / 1000);
				System.out.println(" frames taken until now"+count);
			}
